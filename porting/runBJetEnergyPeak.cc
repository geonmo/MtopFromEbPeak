...
int main( int argc, char* argv[ ]) {
  ...
  std::string outFileURL = argv[2];
  std::string json = argv[3];
  double xsec = std::stoi( argv[4]);
  runBJetEnergyPeak eb_park( inFileURL, outFileURL ) ;

  std::cout<<"...analysing "<<inFileURL<<std::endl;
	return 0;
}
